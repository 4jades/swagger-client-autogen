...
export const setupCodegenConfig = (config: InputCodegenConfig): CodegenConfig => {
	return {
		...
		outputMap: {
			dto: {
				output: getAbsoluteFilePath(
					config.outputMap?.dto?.output ?? "src/shared/api/dto.ts",
				),
				alias: "@/shared/api/dto.ts",
			},
			api: {
				output: getAbsoluteFilePath(
					config.outputMap?.api?.output ??
			...
			apiInstance: {
				output: getAbsoluteFilePath(
					config.outputMap?.apiInstance?.output ??
			...
			mutation: {
				output: getAbsoluteFilePath(
					config.outputMap?.mutation?.output ??
			...
			schema: {
				output: getAbsoluteFilePath(
					config.outputMap?.schema?.output ?? "src/shared/api/schema.gen.ts",
				),
