...
function injectQueryKey(operation, queryKey) {
	...
	for (const key of keys) {
		newOp[key] = operation[key];
		if (key === "tags") {
			newOp["x-query-key"] = queryKey;
		}
	}
	if (!("tags" in operation)) {
		return { "x-query-key": queryKey, ...operation };
	}
	return newOp;
}
		...
		if (key.startsWith("x-")) {
			...
			if (key === "x-query-key") {
				...
				if (existingQueryKey !== newQueryKey) {
					console.warn(
						`⚠️ Query key changed for ${existingOperation.operationId || "unknown operation"}:`,
					);
...
function mergeSwaggerData(newSwaggerData, existingSwaggerData) {
	...
	for (const [operationId, existingOpInfo] of existingOperationMap) {
		...
		if (newOpInfo) {
			...
			const mergedOperation = mergeCustomFields(
				newOpInfo.operation,
				existingOpInfo.operation,
			);

			// 병합된 operation을 새로운 swagger 데이터에 적용
			mergedSwaggerData.paths[newOpInfo.pathKey][newOpInfo.method] =
				mergedOperation;
			...
			console.log(
				`⚠️  Operation ${operationId} exists in local file but not in server response`,
			);
		}
	}
...
try {
  ...
  // GET 요청에 x-query-key 추가
		...
		const targetFilePath = path.resolve(
			process.cwd(),
			`swagger/${toKebabCase(modifiedSwaggerData.info.title)}.yml`,
		);
		...
		if (fs.existsSync(targetFilePath)) {
			try {
				...
				finalSwaggerData = mergeSwaggerData(
					modifiedSwaggerData,
					existingSwaggerData,
				);
				...
				console.warn(
					"⚠️  Failed to merge with existing file, using new data only:",
					mergeError.message,
				);
